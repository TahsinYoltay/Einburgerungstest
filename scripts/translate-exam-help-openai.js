/**
 * Translate firebase_config/exam_help.en.json into other languages using OpenAI.
 *
 * Usage:
 *   node scripts/translate-exam-help-openai.js           # translate ALL target languages
 *   node scripts/translate-exam-help-openai.js fr        # translate only French (code or name)
 *
 * Requires:
 *   - OPENAI_API_KEY in your environment/.env
 *
 * Output:
 *   firebase_config/exam_help.<lang>.json
...
const SOURCE_FILE = path.join(__dirname, '..', 'firebase_config', 'exam_help.en.json');
...
async function translateDoc(doc, languageName) {
  const prompt = `
You are a professional translator for a "Life in the UK" exam help center. Translate the given JSON into ${languageName}.
...
- Preserve punctuation and keep a concise, official tone suitable for government/exam guidance.
...
async function processLanguage(language, sourceDoc) {
  console.log(`\n🚀 Translating exam help for ${language.name} (${language.code})...`);
  ...
  const outPath = path.join(OUTPUT_DIR, `exam_help.${language.code}.json`);
...
async function main() {
  ...
  console.log('\n✨ Done translating exam help files.');
}
