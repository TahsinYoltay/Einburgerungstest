...
import { Card, Text, Divider, List, useTheme, Button, IconButton, Dialog, Portal, Badge, Chip, ProgressBar } from 'react-native-paper';
...
const ExamHistorySummary = () => {
  ...
  const getExamButtonText = (examId: string) => {
    ...
    if (!exam || !exam.lastAttempt) return t('exam.start');
  ...
  const getStatusLabel = (status: string) => {
    ...
    return <Text style={{ color }}>{t(`exam.statusTypes.${status}`)}</Text>;
  };
